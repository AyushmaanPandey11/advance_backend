...
export class PubSubManager {
  ...
  public UserUnsubscribe(userId: string, stock: string) {
    ...
    if (this.subscriptions.get(stock)?.length === 0) {
      ...
      console.log("Unsubcribed from the redis channel: ", stock);
    }
  }
  private handleMessage(stock: string, message: string) {
    console.log(
      `Message received on channel ${stock} and message : ${message}`
    );
...
export const pubSubManagerInstance = PubSubManager.getInstance();
